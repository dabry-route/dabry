...
# def solve_ivp_constrained_maxsrf(ff: FlowField, obs: Obstacle, direction: float, max_srf: float,
#                           t_span: tuple[float], x0: ndarray, t_eval: ndarray, ):
...
class SolverEF(ABC):
    ...
    def dyn_constr(self, t: float, x: ndarray):
        ...
        d = (2. * self.obs_active_trigo - 1.) * np.array(((0., 1.), (-1., 0.))) @ n
        ff_val = self.mp.model.ff.value(t, x)
        ff_ortho = ff_val @ n
        angle = np.arctan2(np.sqrt(self.mp.srf_max ** 2 - ff_ortho ** 2), ff_ortho) - np.pi / 2.
        u = self.mp.srf_max * np.array(((np.cos(angle), -np.sin(angle)), (np.sin(angle), np.cos(angle)))) @ d
        return self.mp.model.ff.value(t, x) + u
    ...
    def step(self):
        ...
        for site in tqdm(self._to_shoot_sites):
            ...
            res = scitg.solve_ivp(self.dyn_augsys, (site.t_init, self.t_upper_bound),
                                  ...
                                  t_eval=t_eval,
                                  events=self._events, max_step=1e-2, dense_output=True)
            ...
            if len(active_obstacles) >= 1:
                ...
                site.index_t_obs = res.t.shape[0] + 1
                ...
                res = scitg.solve_ivp(self.dyn_constr, (t_enter_obs, self.t_upper_bound),
                                      state_aug_cross[:2],
                                      t_eval=t_eval[t_eval > t_enter_obs], max_step=1e-2)
