...
if __name__ == '__main__':
    ...
    perso = np.array(list(map(
        lambda x: proj_ortho(x[0], x[1], DEG_TO_RAD * center[0], DEG_TO_RAD * center[1]), DEG_TO_RAD * points)))
    ...
    perso_inv = np.array(list(map(
        lambda x: proj_ortho_inv(x[0], x[1], DEG_TO_RAD * center[0], DEG_TO_RAD * center[1]), perso)))

    print('Test proj_ortho_inv')
    print(
        f'{np.sum(np.abs(DEG_TO_RAD * points - perso_inv))} (negligible compared to {np.linalg.norm(DEG_TO_RAD * points)} ?)')
    ...
    d_manual = np.array(list(map(
        lambda x: m_diff((
            lambda y: proj_ortho(y[0], y[1], DEG_TO_RAD * center[0], DEG_TO_RAD * center[1])),
            x),
        DEG_TO_RAD * points)))

    d_analytical = np.array(list(map(
        lambda x: d_proj_ortho(x[0], x[1], DEG_TO_RAD * center[0], DEG_TO_RAD * center[1]), DEG_TO_RAD * points)))
    ...
    d_manual_inv = np.array(list(map(
        lambda x: m_diff((
            lambda y: proj_ortho_inv(y[0], y[1], DEG_TO_RAD * center[0], DEG_TO_RAD * center[1])),
            x, dx=1e-2),
        package)))

    d_analytical_inv = np.array(list(map(
        lambda x: d_proj_ortho_inv(x[0], x[1], DEG_TO_RAD * center[0], DEG_TO_RAD * center[1]), package)))
